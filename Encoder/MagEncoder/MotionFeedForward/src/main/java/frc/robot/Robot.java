...
public class Robot extends TimedRobot {
	...
	TalonSRX _leftMasterF = new TalonSRX(1);
	TalonSRX _rightMaster = new TalonSRX(3);
	TalonSRX _rightMasterF = new TalonSRX(4);
	...
	public void robotInit() {
		...
		_rightMasterF.setInverted(InvertType.FollowMaster);
		_leftMasterF.follow(_leftMaster);
		_rightMasterF.follow(_rightMaster);
	...
	{
		/* Gamepad processing */
		double forward = -0.4 * _gamepad.getRawAxis(5);
		double turn = 0.4* _gamepad.getRawAxis(4);
		...
		else
		{
			if (_firstCall) 
			{
				...
				System.out.println("Servo [-4, 4] rotations while also having to ability to add a feed forward");
				zeroSensors();
